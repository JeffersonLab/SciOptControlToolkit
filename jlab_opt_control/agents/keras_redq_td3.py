...
class KerasREDQTD3(jlab_opt_control.Agent):
    ...
    def __init__(self, env, logdir, buffer_type=None, buffer_size=None, cfg='keras_redq_td3.json'):
        ...
        try:
            ...
            redq_log.info(f'Action upper bound: {self.upper_bound}')
            redq_log.info(
                f'Action upper bound: {float(env.action_space.high[0])}')
            redq_log.info(f'Action lower bound: {self.lower_bound}')
            redq_log.info(
                f'Action lower bound: {float(env.action_space.low[0])}')
            self.range = self.upper_bound - self.lower_bound
            redq_log.info(f'Action range: {self.range}')
        except:
            redq_log.error('Action space not valid for this agent.')
            sys.exit(0)
        ...
        redq_log.debug(f'pfn_json_file:{self.pfn_json_file}')
        ...
        # Always use legacy optimizers to avoid variable tracking issues
    ...
    def initialize_new_models(self):
        ...
        for i in range(self.num_critics):
            ...
            # Sleep a bit to make sure each critic gets a different seed
            time.sleep(1 / 10)
    ...
    def train_critics(self, states, actions, rewards, next_states, dones, weights, critic_indices):
        ...
        # Update each critic separately to avoid variable tracking issues
    ...
    def train(self):
        """ Method used to train with higher UTD ratio """
        self.ntrain_calls += 1
    ...
    def save_cfg(self):
        ...
        try:
            ...
            redq_log.info('Agent config saved successfully')
        except:
            redq_log.error("Error in saving the agent cfg...")