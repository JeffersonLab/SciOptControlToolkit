...
class ActorFCNN(Model):
    def __init__(self, state_dim, action_dim, min_action, max_action, logdir, cfg='actor_fcnn.cfg'):
        ...
        self.nodes_per_layer = cfg_data.get('nodes_per_layer', [256, 256])  # Default
        self.activation_functions = cfg_data.get('activation_functions', ["relu"] * hidden_layers + ["tanh"])  # Defaults
        kernel_initializer = cfg_data.get('kernel_initializer', 'glorot_uniform')
 
        self.logdir = logdir

        # Error Checking
        if hidden_layers != len(self.nodes_per_layer) or hidden_layers != len(self.activation_functions)-1:
            if hidden_layers != len(self.nodes_per_layer):
        ...
        if self.activation_functions[-1] not in ["tanh"]:
        ...
        for i in range(hidden_layers):
            ...
            self.hidden_layers.append(layers.Dense(self.nodes_per_layer[i], use_bias=False, kernel_initializer=kernel_initializer, input_shape=(state_dim,) if i == 0 else ()))
            self.hidden_layers.append(layers.Activation(self.activation_functions[i]))
        # Output layer with its specified activation function
        self.output_layer = layers.Dense(action_dim, activation=self.activation_functions[-1])
