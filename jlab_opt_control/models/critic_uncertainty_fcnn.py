...
class CriticUncertaintyFCNN(Model):
    ''' Same as CriticFCNN except it predicts a mean and uncertainty for the Q value, 
        so the output dimension is twice as large'''
    def __init__(self, state_dim, action_dim, logdir, cfg='critic_uncertainty_fcnn.cfg'):
        super().__init__()
       ...
       # Error Checking
        ...
        # Output layer
        self.output_layer = layers.Dense(2, activation=activation_functions[-1])  # Last activation function for output
    ...
    def call(self, state, action, training=False):
        ...
        return x[:, 0:1], x[:, 1:2]  # Return mean and variance
