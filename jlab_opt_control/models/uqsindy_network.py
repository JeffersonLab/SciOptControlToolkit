...
class UQSINDyNetwork(Model):
    ...
    def __init__(self, num_features_in, num_features_out, min_action=None, max_action=None, batch_size=128, logdir='./results', cfg='uqsindy_network.cfg'):
        super().__init__()
        ...
        if max_action!=None and min_action!=None:
    ...
    def sample_posterior(self, sampling_size=1024):
        ...
        randn = tf.random.normal(shape=(sampling_size, *self.mu.shape), dtype=tf.float32)
    ...
    def call(self, inputs, nsamples=0, training=False):
        ...
        #print(f'BX: {BX.shape}')
        if self.using_tanh:
            BX = tf.keras.activations.tanh( BX[0,:,:] )
